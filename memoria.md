...
### FOTOGRAMA V3 — compilador semântico, referências reais e quality gates
- **Causa do output genérico com 3 assets**: ambiente, contact sheet de personagem e prancha multivista do produto eram enviados como imagens equivalentes, seguidos por uma regra genérica de “products/subjects”. O modelo precisava adivinhar o papel de cada uma. Agora cada ref tem função `Auto / Ambiente-look / Personagem / Produto / Figurino / Composição`; uma passagem de visão classifica e descreve, e o payload intercala `REFERENCE N — ROLE` imediatamente antes da imagem correspondente. Contact sheet = **uma identidade**, multi-view = **um produto**, nunca grupo/collage.
- **Diretor confirmado e redesenhado**: Gemini Flash continua antes do modelo de imagem, mas agora a passagem de linguagem roda SEMPRE. Checkbox ligado acrescenta produção/direção de arte somente a briefs esparsos; desligado traduz e limpa literalmente. Cena e Ficha falham fechado se a normalização voltar malformada — português cru não é mais enviado silenciosamente. O modelo de imagem recebe partes multimodais + um prompt final limpo, não o `systemInstruction` do Diretor.
- **Compilador V3 por slots**: cena > Ficha > Emulsão dominante nos slots livres > seletor explícito > Auto do programa, uma voz por escala, ponto de vista, ótica, profundidade, luz, cor e textura. Programas/gêneros deixaram de ser blocos monolíticos; mortos os conflitos f/16+bokeh, HP5+grade colorido, Street coarse+ultrafine, Music Video+Muted/Golden e ambiente+paleta explícita.
- **Câmera**: `Escala do plano` foi separada de `Ponto de vista`; Worm's Eye não é mais sinônimo de crop. Personagem em pé/levitando recebe cobertura head-to-toe, headroom/footroom, mãos/pés/cabeça dentro do quadro e proibição de pose sentada/reclinada. A UI avisa que Worm's Eye direto privilegia céu/sola; Low Hero preserva melhor um platô legível.
- **Payload/snapshot**: prompt exibido = último texto exato enviado; fallback recompila por modelo; fila congela modelo/AR/resolução/parâmetros/refs/Ficha; Nano Banana 2 recebe `imageSize` 2K/4K; Lite não é fallback silencioso; última imagem final não-thought é escolhida. Inputs pequenos sobem a 2048 e PNG transparente não é achatado em preto. No Pro, Emulsão usa descrição medida; se a análise falhar, a STYLE ref em alta é anexada em vez de desaparecer.
- **Ordem da ferramenta**: Estética → Cena → Referências → Aspect Ratio → Escala → Ponto de vista → Lente → Profundidade → Emulsão → Luz → Stock → Paleta → Ficha → Modelo → Resolução → Prompt → Gerar. A ordem de clique não altera o resultado: o compilador resolve apenas o snapshot atual. Mobile: essenciais abertas, disclosures próprios sem duplo estado, alvos de toque 44px.
- **Testes mockados, zero créditos/rede real**: `test-fotograma-v3` 110, `prompt` 48, `res` 24, `scene` 24, `vertex` 5 — **211/211 ALL PASS**, zero `pageerror`. Inclui o caso real platô + personagem nórdico + óculos, referências intercaladas, anti-crop, prompt=pkg, 2K, fail-closed, PNG, mobile e toda a matriz fotoquímica descrita abaixo. Pendência honesta: A/B visual com gerações reais para calibrar as assinaturas; não realizado nesta sessão.

### FOTOGRAMA V3.1 — densidade fotoquímica, highlights protegidos e grão inegociável
- **Regras da casa em todas as Estéticas**: highlights preservam matiz e textura sob um shoulder de película — fontes podem bloom/halate ao redor de um núcleo legível, nunca virar branco amplo sem informação. O grão fotoquímico orgânico permanece perceptível no tamanho final, irregular, dependente da exposição e embutido na luminância; `clean` significa película fina, nunca digital liso.
- **Exposição default mais densa**: Cinema, Music Video e os oito gêneros de Publicidade foram recalibrados com iluminação localizada, fill contido, médios com peso e pretos profundos porém separados. Cinema Auto passa a grão 35mm médio-pesado; Publicidade preserva caráter por segmento — fino-médio em moda/beleza, médio em geral/auto/food/esporte/tech e grosso em streetwear — sem apagar a película.
- **Arbitragem sem conflito**: Cena/Ficha que declaram exposição ou contraste ocupam a tonalidade inteira e desligam o baseline e os componentes tonais Auto concorrentes. Luz/paleta escolhidas explicitamente preservam apenas direção e matiz via variantes `source`/`hue`; stock preserva só seu caráter de grão via `grain`. `ambient light`, `available light` e `negative fill` não são mais falsos overrides de exposição.
- **Emulsão confinada aos slots livres**: a análise rotulada é filtrada em `COLORS/TEMPERATURE`, `LIGHT`, `TEXTURE` e `MOOD`; não vaza luz/textura sobre a Ficha. A Emulsão vem antes das duas invariantes finais e a STYLE anexada é explicitamente subordinada ao shoulder e ao piso de grão. Descrição legada inseparável faz fallback para a imagem STYLE em alta em vez de prometer uma transferência sem dados.
- **Pedidos incompatíveis falham de forma criativa**: `sem grão/no grain` vira limpeza fotoquímica de grão fino; `highlights estourados/blown-out` vira brilho com shoulder e informação. A sanitização cobre PT/EN sem corromper conteúdo como `clean digital display`.
...
### FOTOGRAMA — PUBLICITÁRIO v2.1: pesquisa do Daniel aplicada (fichas por gênero + moodboard ShotDeck + Angénieux)
- **Fontes** (entregues 23/07): `_knowledge/fotografia/Visual Directives for Advertising Photography and Cinema.md` (fichas técnicas por gênero 2015-2026 — a pesquisa NotebookLM que pedi no brief), `The Angénieux Visual Signature_ AI Prompting Guide.md`, e 22 frames ShotDeck em `Desktop/MOODBOARDS/06 - Publicidade` (estudados um a um).
- **DNA do moodboard fundido no commercial.scene**: câmera perto e imersiva no meio da ação, carisma direto pra lente, figurino/set com cores deliberadamente coordenadas (production design), humor candid em mundo estilizado, pele real com suor e sardas. (Padrões dominantes nos 22 frames: POV/low-angle íntimos, golden/janela com punch, teal-quente, grão vivo.)
- **Gêneros refinados com as fichas**: moda ganhou flash-on + gleam de materiais + poses esculturais; auto ganhou compressão tele + sombras baixas profundas ("NUNCA clarear a mecânica baixa") + golden/overcast pra pintura honesta; food ganhou high-key + pours congelados 1000fps + blacks controlados; beleza virou glass-skin/transparência radical; esporte ganhou rim light dura esculpindo músculo + energia descentrada; tech ganhou dark-mode blacks + speculars afiados + neon bioluminescente. **+ STREETWEAR novo** (flash cru 2000s disposable, grão pesado, lo-fi paparazzi, chartreuse elétrico).
- **Music Video refinado**: haze volumétrico/god rays/light leaks/caos controlado no scene; sombras podem esmagar (fixed); **lensAuto** novo — 'Programa decide' na lente agora deixa o programa opinar (clipe → 'vintage anamorphic glass, oval bokeh, horizontal streak flare'; buildFinalPrompt: `lens.phrase || P.lensAuto`).
...
### FOTOGRAMA — AUDITORIA COMPLETA vs template Cinema Studio 2 + controle total (Daniel 23/07)
- **Fonte**: `nano banana testing/vault_nano banana/_knowledge/system prompts/SYSTEM PROMPT CINEMA STUDIO 2.jpeg` (lido por Spotlight — o caminho que o Daniel citou não existia). Mapa slots→Tipó: FOCUS_DIST≈FRAMINGS ✓, LENS≈LENTES ✓, CAMERA_MODEL≈STOCKS+texture-override ✓, SCENE DIRECTION≈FRAMINGS+LUZES ✓, SURFACE&GRADE≈PALETAS+P.fixed ✓, SUBJECT≈`<user-input>` ✓ (só no caminho Diretor — o prompt final vai pro modelo de IMAGEM, injection lá não executa), CORE A/B≈refs/geração ✓ parcial, **APERTURE_POINT = BURACO** (não existia DoF).
- **Arquitetura confirmada na auditoria**: o "modelo saber quando usar cada tag" NÃO é o nosso desenho — os slots entram DETERMINISTICAMENTE por código (a LLM nunca decide seletor); o Diretor só decupa a CENA. É mais forte que o template (que confia no modelo seguir pipeline).
- **PROFUNDIDADE (novo seletor, APERTURE_POINT)**: `APERTURAS` auto/f1.2 sonho raso/f2.8 isolado/f8 equilíbrio/f16 deep focus — branching do CS2 (f baixo = bokeh pesado isola; f alto = tudo nítido). Ótica pura: emulsão NÃO cala; some na publicidade (gênero comanda ótica).
- **Anti-preguiça de refs (CORE A do CS2)**: refsClause ganhou "The reference locks identity — NOT composition: angle, framing, scale and action follow the scene text; recompose boldly, do not be lazy".
- **Cerveja fantasma no food** (print do Daniel: copo dourado em TODA cena do gênero): a frase determinística tinha 'backlit translucency in liquids, condensation' → o modelo servia bebida sempre. Fix: frase enxuta ('appetite-driven... textures glistening, crumb readable'), líquido/condensação foram pros instintos do Diretor com trava dupla: 'ONLY on drinks the scene itself names' + 'NEVER add uninvited dishes or beverages'.
- **Diretor**: cláusula **ART DIRECTION IS A DELIVERABLE** aplicada (adjetivos de estilo/época — elegante, excêntrico, Y2K — e direção de arte de marca são BRIEFS: materializar em materiais/texturas/mobiliário/figurino/organização; 'never leave a style word as a bare adjective'). Diagnóstico dos prints ruins do Daniel: o Diretor tava DESLIGADO (o ↺ reusar restaurava diretor:false de take antigo silenciosamente). Fix: **applyParams não mexe mais no checkbox** — Diretor é preferência viva, nasce ligado. Prova real dele: Y2K McDonald's com Diretor ligado saiu cromados+neon+figurino prata ("já melhorou").
- **PROMPT FINAL ao vivo** (o "controle total / exata noção do que vai ser gerado"): seção nova com toggle 'ver' → mostra o buildFinalPrompt montado EM TEMPO REAL (qualquer input/chip/select refresca; early-return se fechado) + avisos: texto traz ENQUADRAMENTO/LUZ → seletor mudo; emulsão no comando; publicidade = gênero comanda; Diretor vai decupar. Botão copiar.
- test-fotograma-prompt: 33 checks ALL PASS (novos: f/16 no prompt, apertura some em ad, food sem líquido, uninvited no Diretor, ART DIRECTION, refs anti-lazy, diretor sticky, preview abre/avisa/mostra). 4 suítes ALL PASS.
- ~~Pendente~~ → **MODO FICHA IMPLEMENTADO** (Daniel aprovou 23/07): seção "Ficha técnica" colapsada (badge ✍N no botão quando tem campo ativo) com 6 inputs livres — distância/focal, ótica, profundidade, luz, textura/grão, grade/cor. **Preenchido SUBSTITUI o seletor correspondente** (`fichaVals()` no buildFinalPrompt: `F.x || (regra normal)`) e **vence até na publicidade e na emulsão** (mão do usuário > gênero > seletor). Textura preenchida também mata o texture-override do stock. Snapshot/applyParams levam a ficha (↺ reproduz), legenda mostra "ficha ✍N", preview avisa "N campos no comando".
- **DRAG galeria → REFERÊNCIAS/EMULSÃO** (mesmo pedido): take da galeria é draggable (dataTransfer `application/x-tipo-take` com o id); `bindDrop` das duas seções checa o tipo custom ANTES dos files — acha o take em state.takes, embrulha o blob em File e entra no funil normal (addRefFiles/setMoodFile). Gerou→arrasta pra emulsão = iterar em cima do próprio output.
- test-fotograma-prompt: **41 checks ALL PASS** (ficha substitui/some seletor, vence na publicidade, snapshot+badge, applyParams limpa, take draggable, drop vira ref, drop vira mood). GOTCHA de teste: setar `.value` via JS não dispara 'input' — badge sincroniza via delegação de input + dispatch no teste.
...
### Fotograma — interface Image Lab finalmente visível
- **Correção de rumo:** o commit `cfdb4b1` estava publicado e byte a byte idêntico ao Vercel, mas a mudança visível era apenas um dropdown discreto; não entregava a interface por ferramentas vista na referência. A nova versão adiciona rail permanente e painéis próprios no `fotograma.html`.
- **Superfície aprovada:** Create preserva o Fotograma cinematográfico; Multi Angle chama Qwen Camera Control; Animation oferece 3D Film, Feature 3D, Stylized 3D, Modern 2D, Anime Film e Clay; Expand chama Outpaint por aspect ratio; Remove BG aparece como beta; Depth Map fica acessível como ferramenta irmã. Upscale e Expand por pixels seguem fora.
- **System prompts:** Animation usa prompts autorais em `shared/fotograma-tools.js`, construídos pelo comportamento observado e pelas necessidades da Tipó — fidelidade de identidade, pose, roupa, objetos, perspectiva, composição e luz — sem copiar prompts privados da plataforma de terceiros. Multi Angle, Expand e Remove BG são modelos especializados e não inventam prompt decorativo.
...
### Fotograma — Cast, Product, Sheets e inspector expandido
- **Bloco novo:** sidebar ganhou Cast, Product e Sheets. Os três passam pelo `/generate` já endurecido do bridge Higgsfield — não ampliam a superfície do backend e continuam com um único job por clique, sem retry/fallback pago.
- **Prompts autorais:** `shared/fotograma-tools.js` agora concentra taxonomias e builders próprios. Cast trava uma identidade humana canônica; Product trava geometria/material/branding; Sheets exige a mesma identidade ou o mesmo produto em todos os painéis. Nenhum prompt privado da referência foi copiado.
- **Cast:** pode nascer só de texto ou combinar descrição + imagem. Quatro linguagens (Natural/Cinematic/Editorial/Advertising), quatro fundos e default Nano Banana Pro 3:4 2K.
- **Product:** pode nascer de descrição ou preservar uma referência; presets Studio/Campaign/Lifestyle/Macro e default Nano Banana Pro 1:1 2K. Rótulo legível existente é preservado; texto novo não é inventado.
- **Sheets:** sempre exige origem; Character 360°, Expressões, Poses e Produto 360°, default Nano Banana Pro 16:9 2K. O inspector tem handoff `Usar no Sheets` que carrega o blob local diretamente como nova origem.
...
### Hotfix — falso timeout de 30 segundos no Higgsfield
- **Falha nossa:** o adapter instanciado no pareamento recebia `timeoutMs: 30_000`. Isso servia para o health, mas o mesmo objeto era guardado em `state.higgsAdapter`, então `/generate` era abortado pelo browser após 30s apesar do bridge/CLI continuar trabalhando e consumindo o crédito.
- **Correção:** `HiggsfieldBridgeAdapter` ganhou relógios separados: health 30s; geração 23min. O bridge continua com timeout interno de 22min, garantindo que sua resposta chegue antes do abort do navegador. Query de cache atualizada para `20260825-higgsfield4`.
- **Evidência real sem retry:** durante o diagnóstico, o bridge mostrava um job ativo mesmo depois do erro visual; depois ficou ocioso e o histórico confirmou duas saídas do brief de Udine concluídas (Seedream 5 Pro e GPT Image 2). Saldo observado 1255,87 → 1245,87. Nenhuma nova geração foi disparada por nós.
- **Teste:** `test-fotograma-higgsfield.mjs` falhou primeiro com `{generation:30000}` e passou depois com `{generation:1380000,health:30000}`, além de toda a cobertura existente de no-retry/no-fallback/segredo/mobile.

### Fotograma — revelar continuamente e preservar toda a galeria local
- **Problema percebido:** `Revelar Fotograma` ficava desabilitado enquanto uma geração rodava, então o grid dava a sensação de suportar somente um lote por vez. Além disso, o IndexedDB guardava imagens, mas o reload restaurava apenas as 30 mais recentes; durante a sessão ainda havia uma remoção automática depois de 120 takes.
- **Fila contínua:** o botão permanece utilizável e passa a dizer `Revelar +N fotogramas`. Cada clique cria um snapshot independente da cena, modelo, referências e parâmetros daquele momento. Quatro jobs rodam simultaneamente; os próximos aparecem imediatamente como `na fila · posição N` e avançam sozinhos. O teto é 48 imagens ativas/aguardando para impedir fila paga acidentalmente ilimitada.
- **Persistência realista:** todos os registros do IndexedDB voltam após reload e nenhum take é excluído automaticamente pela aplicação. A página solicita armazenamento persistente ao navegador, usa `loading=lazy`/`decoding=async` e avisa quando a quota impede salvar. Isso permanece local ao domínio + navegador + perfil; não sincroniza entre aparelhos, janela anônima não compartilha o histórico e limpar dados do site remove tudo. Saídas importantes devem ser baixadas.
- **Prova:** teste end-to-end adicionou lotes consecutivos durante o processamento, confirmou quatro ativos + excedente esperando, recebeu 12 arquivos, garantiu Diretor exatamente uma vez por clique mesmo com lote parcialmente enfileirado e recarregou 38 imagens sem corte. Higgsfield, ferramentas e prompts continuaram ALL PASS. Nenhuma chamada real ou crédito foi usado.

---

## 2026-09-05

### Fotograma — publicação expressamente autorizada
- Após a explicação de que os looks já estavam implementados, mas o filtro não tornava o código privado, Daniel pediu “faz push e vamo ser feliz”. A suspensão registrada abaixo foi encerrada por essa instrução; publicar o bloco atual sem alegar backend privado ou receitas secretas.
- Escopo: refino visual/3 modos/galeria setas+A/B/input original/Uso/guard; sem mudança de modelo, credenciais, vínculo de contas ou processo pessoal da bridge. Backend privado continua pendente. Seis PNGs pessoais e estudos privados ficam fora do Git.
- Gate de release: 1.413 verificações prévias + auditoria; nova revisão/smoke antes do commit e `test-production.mjs` depois do deploy para 34 arquivos, 41 ferramentas e desktop/mobile sem provedores. Base anterior `b1ec154`, rollback por revert se houver regressão. Não gerar imagens nesta liberação.

### Fotograma — refino das referências e prioridades (local, posterior à rodada abaixo)
- Daniel aprovou melhorar com o material já levantado e depois fazer push. Foram usadas somente leituras originais das seis capturas fornecidas; não alegar estudo do catálogo inteiro do ShotDeck. Nenhum acesso/coleta adicional nesta rodada. Capturas, análise particular e baseline de prompts ficam fora do Git.
- Audiovisual-squad orientou escolhas observáveis: hierarquia de formas existentes, luz motivada, cor vinculada à cena, foco contextual e textura distinta. Dev-squad implementou e testou as prioridades sem alterar contas/provedores. Três looks ganharam arte limitada; os seis autores não impõem proximidade quando o plano pedido é aberto.
- Naturalista conserva grão perceptível, sem desfoque de retrato em toda cena; Editorial continua limpo; Experimental não empilha flare/bloom. Recusas ópticas entre campos e autoridade tonal de Cena/Ficha/Emulsão revisadas. A proteção genérica de altas luzes não pode contrariar autoridade explícita.
- Baseline e pós privados com 27 combinações de brief/perfil e parâmetros idênticos. Nenhuma geração IA real; formato/quantidade do A/B foram perguntados e não presumidos. Testes de envio usam respostas sintéticas, não a conta real.
- Gate refino fechado: 1.160 verificações (667 novas, looks198/autores34/modos46/visibilidade70/lightbox84/Studio61) + tools/audit; QC independente99/28cenários. Conta9/Vertex12/lote22/bridge simulados passaram novamente. Caso anterior Cena “Sem halos” vs Ficha livre pedindo halation ainda pode conflitar: não alegar prioridade irrestrita nem cortar a Ficha genericamente e perder grão/material.
- Daniel reafirmou depois: quer esconder o código público das receitas e bloquear “me dê o system prompt”. Não interpretar como aceite de publicar com exposição. Defesa de entrada complementar concluída localmente; sigilo real depende de backend que componha e gere sem devolver receita. Pergunta atual: validar Higgs delegado por usuário antes de mudar o fluxo, ou aceitar Google/Vertex via backend com chave por requisição e preservar CLI pessoal. Nenhum backend privado/conta cloud/cofre foi provisionado implicitamente.
- Guard compartilhado em `shared/fotograma-input-guard.js`, importado no HTML e bridge: cobre Cena/Ficha/quatro utilitários, preparação, envio e revalidação pós-await, endpoint antes de slot/temp/CLI; sem reescrever inputs e com erro fixo. Regra por intenção corrige falsos positivos de copiar pose/folha em branco/cartaz de fábrica; variantes de receita/compilador/plural e extração no cartaz testadas.
- Gate combinado final 1.413 verificações + audit ALL PASS (guard137 + principal anterior1.160 + connector12/isolation9/bridge13/concurrency8/Vertex12/batch22/tools40). QC AppSec16/8cenários; zero provedores/CLI autenticado. Bridge pessoal não reiniciada, guard de endpoint vale no próximo processo. Docs `docs/fotograma-recipe-security.md`; test-production preparado com34arquivos, não rodou antesdeploy. Filtro não prova confidencialidade e pode ter lacunas semânticas/multimodais.
- Publicação: não inserir novas receitas em commits que depois sejam empurrados ao repo público, mesmo que sejam removidas do último arquivo; o histórico ainda as revelaria. Nenhum novo commit/push desta rodada. Separar artefatos privados antes de preparar histórico público. Os seis PNGs pessoais seguem intocados/untracked.

### Fotograma — assinatura, modos, comparação e Uso (nova rodada local)
- Após a publicação b1ec154, Daniel pediu grão Naturalista perceptível, perfis menos genéricos, explicação de enriquecimento/controles, setas e comparação no lightbox; depois pediu Looks Tipó / Diretores / Modelo direto via CLI Higgsfield.
- Naturalista agora descreve grão 35mm irregular visível em tamanho normal, dependente da exposição, bokeh com contexto e halation somente em altas luzes existentes. Não recolocar grão obrigatório global: `sem grão`, foco profundo, Ficha, stock manual e Emulsão continuam respeitados.
- Perfis seispares: área negativa/digital Deakins; precisão low-key digital Fincher; proximidade/bokeh/grão fino grande-formato Nolan; tableau/foco profundo/grão fino Anderson; amplitude orgânica/luz disponível Malick; diagonais/profundidade documental/grão pequeno-formato Charlone. Direção de arte top-level separada, sem inventar elementos nem escapar dos slots de cor/luz/textura. Fontes/evidência vs interpretação em `docs/fotograma-auteur-sources.md`.
- `directionMode` preserva signature/auteur/standard. Botões públicos3caminhos; standard agora rotulado Modelo direto. Só clique intencional seleciona Higgsfield; não opta por conta nem altera provedor em boot/applyParams. Modelos reais e estado de conexão visíveis; Vertex continua disponível em Conexões. Não mudou nem chamou CLI/contas reais.
- Enriquecer off mantém cena literal com direção local; on somente ideias≤350 com Google, sem mudar fotografia, original preservado; standard nunca enriquece. Clique de preset substitui categoria, não acumula. Emulsão≥50 continua anulando seletores luz/stock/paleta conforme regra anterior; Cena/Ficha explícitas vencem. Avisos novos tornam interferências visíveis.
- Lightbox anterior/próxima/←→ com ordem filtrada, contador e limites; comparar A atual/B fixa distinta, ações sóA, contain desktop/mobile. Exclusão corrigida para aguardar commitdurável antes de sumir; testes cobremreloadimediato, foco, filtros e novasimagens.
- Daniel pediu retirar a leitura de prompts das imagens e simplificar Prompts & uso. A UI agora tem somente **Uso**, com contagem local por provedor/ferramenta/modelo, sem catálogo técnico nem abas de receitas. Removidos botões, painéis e callbacks de prévia/cópia/leitura em Create, utilitários, cards e lightbox; não foram apenas escondidos por CSS.
- `galleryOriginalBrief(take)` fornece o input original aos cards e à busca: `params.scene` no Create e `params.brief` nos utilitários. Não há fallback para prompt compilado; take antigo sem input original mantém brief vazio. `take.params.prompt` histórico continua intacto no IndexedDB/reload; nenhuma imagem ou dado pessoal foi apagado. A/B, setas, download, curtida, reuso, Sheets e conexões preservados.
- Gates desta revisão: looks 198, modos 46, lightbox 84, auteur 34, Studio 61, prompt-visibility 70, tools e audit ALL PASS; nenhum erro de página ou chamada a provedor. Os dois novos checks cobrem brief/busca de utilitários. Regressões anteriores: contratos 28, contas 9, Vertex 12, batch 22, reconnect 51. Mocks, nenhuma imagem gerada/cobrança; teste comparativo visual pendente.
- **Publicação adiada durante alinhamento da arquitetura de prompts:** repo confirmado PÚBLICO via GitHub. Retirar a visualização é ajuste de UI, não proteção de confidencialidade: instruções continuam no cliente/repositório e prompts históricos no armazenamento local. Daniel descreveu o fluxo desejado: input simples do usuário → backend com instrução do preset → Nano Banana Pro → galeria somente com input original. A migração e sua compatibilidade com Higgsfield CLI local ainda estão em avaliação; não foi implementado compilador privado nem apagado histórico. Não fazer push sem resolver a escolha sobre exposição das receitas; checkpoint Git local não publica nem protege código cliente. Preservar os seis PNGs pessoais untracked.
- Limite auditado: compilar na bridge local remove as instruções do JavaScript público, mas não as protege do dono do computador. O backend Tipó precisaria compor e enviar ao provedor sem retornar a receita ao cliente. Migração das contas privadas e hospedagem ainda não definidas; auditoria de arquitetura não equivale à implementação.
...
- Gate final: looks171, Studio67, contratos28, isolamento9, tools40, audit5blocos, Vertex/batch/reconnect ALLPASS, GIF32, framingFullHD86, tonal73, motion67, workspaces8, home28, previews38. Corrigido erro real no ensureHiggsfieldConnection: retorno silencioso perdia401 e convertia sessão expirada em transporte; agora mantém Entrar e não repete health/cobrança. Testes com provedores mockados, nenhuma geração IA real.

### Fotograma — revisão antes da liberação
- Daniel suspendeu commit/push para revisar os prompts, pediu perfis diretor+DP (referência Image Pro do colega), contas fora dos parâmetros e isolamento para visitantes; Magnific se possível.
- Vídeo de 2min14s inspecionado. Image Pro (Runware) é rótulo, não AIR ID identificado; não copiar por palpite nem alegar acesso aos prompts privados.
- Implementação naquela revisão: `shared/fotograma-direction.js` seis pares documentados e `shared/fotograma-studio.js/css` com Prompts & uso, sistemas por etapa, prompt salvo, perfis/fontes e contagem local (não fatura). Assinatura / somente brief / diretor+fotografia. O painel de leitura foi posteriormente retirado; a rodada local acima mantém somente Uso.
- Removida reescrita forçada de “sem grão” e clipping; sem baseline obrigatório de subexposição. Ficha literal, sem tradução extra; enriquecimento Google só para ideias ≤350 com toggle ligado, preservando original. Conteúdo primeiro e STYLE depois, sem ambiguidade de numeração. Utilities independentes, contratos menores e 12k preservados.
- Conexões no topo; perfil novo não probe/conecta sozinho. Flag legado mantém Daniel conectado. Disconnect/forget invalidam retornos tardios e abas; ensure não pode reautorizar sem consentimento. URL/bind bridge só loopback. Não reiniciamos bridge nem mexemos na sessão do CLI. Mesmo Mac/perfil compartilhado NÃO tem isolamento multiusuário.
- Testes: Studio67, contratos28, isolamento9, connector12, bridge13 e regressões audit/tools. Sem geração real/cobrança; avaliação visual comparativa ainda pendente. Relatório `docs/audit-fotograma-prompts-2026-09.md`.
- Magnific oficial atual tem REST/MCP OAuth, mas créditos Legacy não confirmados. Pergunta enviada sobre qual conta Daniel tem; conector não implementado nem falsamente habilitado. Proposta `docs/fotograma-magnific-integration.md`, preservar original no futuro upscale (fluxo atual refs reduz a2048). Publicação foi posteriormente aprovada; Magnific continua pendente, não habilitar sem validar a conta.

### Imagem & Efeitos — preview e auditoria para uso em vídeo
- Daniel pediu todas as fontes ampliadas como ASCII e efeitos/presets utilizáveis em vídeos profissionais, sem receitas de meme. Auditoria das 18 ferramentas, com squads dev/design e revisão independente.
- Preview Preencher é default, sem deformar fonte; Ajustar mostra o quadro inteiro. Sete motores dimensionados pela fonte usam `TipoMediaViewport` (Dithering/Riso/Gradient Map/Pixel Sort/Datamosh/Rastro/Depth), separando display e resolução. Não voltar ao CSS max-width que deixava vídeos em miniatura. Zoom não altera export; máscaras/interação continuam nas coordenadas do canvas. Outros motores mantêm seus enquadramentos explícitos.
- Dithering ganhou Tonal/Print/Scanlines/Duotone completos, grid160 inicial, Bayer com amplitude de meio nível e extremos preservados; SVG usa a geometria real (inclusive uploads não quadrados) em vez de círculos. SVG alerta quando glow/tint não pode ser representado. Crop social respeitado no HQ por hook opcional `getOutputSize`. Upload/webcam não substituem fonte durante gravação/HQ, e permissão tardia de câmera não vence upload mais novo.
- ASCII sem distorção/dupla atenuação; Retícula linhas/rings/malha/meios-tons corrigidos; Riso ângulo/grão HQ estáveis; Gradient Map não anima cor sozinho. Glitch RGB não superexpõe, presets atômicos; Pixel Sort sem recortes aleatórios novos; Datamosh limpa buffers; Rastro amostra ecos pelo tempo e protege memória; Overlay procedural determinístico.
- AudioType preserva imagem ao aplicar preset, não deforma, abre Mono e grava trilha de áudio. Pattern SVG XML/texto válido; Palette/Mockup uploads mais seguros; Studio modal acessível; Depth não aplica inferência obsoleta. Fotograma/provedores/galeria preservados, com regressão sem geração paga.
- Full HD real validado nos dez filtros HQ, mais PNG/SVG decodificados, presets, mobile e regressões. Pipeline continua SDR8bit/H26430fps; Cross-Mosh com Vídeo B somente gravação ao vivo; Rastro Alpha não segmenta automaticamente. Relatório principal `docs/audit-image-effects-2026-09.md` reúne evidências e limites dos 18, sem promessa de HDR/ProRes/ICC ou desempenho em takes longos.
...
- **Login sem código:** o CLI atual usa OAuth 2.0 PKCE com callback local. O novo `POST /auth/login` executa somente `higgsfield auth login`, serializa tentativas e devolve apenas status/plano/saldo; senha, access token, refresh token e e-mail nunca chegam ao browser. Na UI, sessão expirada revela um único botão **Entrar**.
- **Sempre ligado no Mac:** `support/macos/install-higgsfield-bridge.zsh` materializa e instala `ai.tipo.higgsfield-bridge.plist` com `RunAtLoad`, `KeepAlive`, `ThrottleInterval`, ambiente explícito, umask 0077 e bind em loopback. O instalador valida a sintaxe antes de tocar no serviço, repete o `bootstrap` durante a curta transição do `bootout`, só confirma depois de `/live` e restaura o plist anterior se a atualização falhar. O processo ocioso foi encerrado de propósito no teste: launchd elevou `runs` e iniciou um PID novo, e `/live` + `/health` voltaram saudáveis sem intervenção.
- **Supervisor da página:** conexão automática no boot e nas ferramentas, health single-flight, backoff exponencial limitado, heartbeat de 90 s e retomada em `focus`, `pageshow`, `online` e volta da aba. Health recente é reutilizado para evitar churn de subprocessos; heartbeats da mesma URL preservam a identidade do adapter para classificar corretamente falhas de gerações longas. Sessão expirada pausa polling até o clique em Entrar e não abre popover quando Google é o motor ativo.
- **Fila sem perda e sem duplicação paga:** se a conexão falha no health anterior ao POST, todos os cards voltam ao estado `aguardando conexão`, conservam a preparação do lote e são retomados uma vez após o health verde. O scheduler mantém FIFO e limites independentes (4 Higgsfield + 1 Google), portanto um Higgs pausado não bloqueia Google. IDs monotônicos evitam colisão entre cliques no mesmo instante. Falha de transporte depois que `/generate` começou permanece terminal e nunca é repetida; erro tardio de adapter antigo não derruba uma conexão nova. 400/409/429 mantêm a sessão.
- **Rede e segurança:** a permissão de **Acesso à rede local** do Chrome continua sendo o único consentimento adicional inevitável para uma página HTTPS falar com loopback. POST exige Origin permitido e apenas `https://tipo-steel.vercel.app` entra por padrão; localhost/desenvolvimento exige opt-in. OAuth bloqueia jobs concorrentes, descarta sucesso de URL antiga e mantém um minuto de folga entre timeout servidor/cliente. `/live` mede liveness e `/health`, autenticação/readiness.
- **Validação sem créditos:** suíte nova cobre boot, queda/retorno, fila antes do envio, identidade exata de cada job, relógio congelado, fila mista, deduplicação/freshness, auth expirada + login em um clique, troca de URL, erro tardio, Higgs em segundo plano, 429 e no-retry do POST pago. Suítes de providers/ferramentas/lotes/bridge/concorrência passaram; smoke Chromium real conectou, leu a conta e habilitou o botão sem chave Google, falhas de request ou pageerrors. Nenhuma imagem foi gerada.
