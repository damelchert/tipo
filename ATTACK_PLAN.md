...
- **23.1 UX one-click ✅ (2026-08-12):** upload agora abre o vídeo como preview principal grande, reproduzível e com controles; durante o job ele vira monitor pequeno e o depth assume o palco. `Generate depth MP4` faz análise → encode → download automático em um clique. Cancel ocupa linha própria logo abaixo e vale durante modelo/análise/encode (cooperativo entre inferências, sem arquivo parcial). `Download again` aparece só depois do sucesso. E2E atualizado ALL PASS, incluindo cancel real, playback desktop/mobile e modelo real até MP4.
...
- **Validação**: bateria de 8 imagens reais via Higgsfield CLI (registrada à época como Unlimited; o CLI atual consome créditos) — 4 cenas × 2 programas com framings não-convencionais; look confirmado (halation CineStill, knee-level com criança na altura da lente, bird's eye com grid, campanha top-down). Demo still da bailarina embarcado (224KB). Smoke mockado 12/12.
...
- ✅ 21.8b (2026-07-23) — pesquisa NotebookLM do Daniel APLICADA: fichas por gênero fundidas nos seeds (+ gênero STREETWEAR novo), DNA do moodboard ShotDeck (22 frames) no commercial.scene, Music Video refinado (haze/leaks/caos + lensAuto anamórfica), lentes **Angénieux Optimo + 25-250 vintage**. Candidato restante: Travel/Airline.
- ✅ 21.8c (2026-07-23) — **Publicidade ISOLA os presets** (print do Daniel: Neon mista → golden hour; presets brigando): Lente/Luz/Stock/Paleta somem da UI e do prompt no programa Publicidade — o gênero é a única voz de ótica/luz, `commercial.fixed` virou voz de pós dominante ("graded like a big-budget campaign in post"), legenda mostra "grade de campanha". Luz específica = escrever na cena (texto é lei).
- ✅ 21.8d (2026-07-23) — **Auditoria vs Cinema Studio 2 + controle total**: seletor PROFUNDIDADE novo (APERTURE_POINT — o buraco achado no mapa de slots), anti-preguiça nas refs (identidade ≠ composição), cerveja fantasma do food morta (líquido saiu da frase determinística), cláusula ART DIRECTION no Diretor (estilo/época/marca = brief materializado), Diretor sticky (↺ não desliga mais), **painel PROMPT FINAL ao vivo** com avisos de contradição. 33 checks.
- ✅ 21.8e (2026-07-23) — **MODO FICHA**: 6 campos livres (distância/ótica/DoF/luz/textura/grade), preenchido substitui o seletor (vence até publicidade/emulsão), badge ✍N, snapshot/↺/legenda/preview integrados. + **DRAG galeria→refs/emulsão**: take arrastável vira referência ou mood (iterar sobre o próprio output).
#### 21.9 — PROVIDER HÍBRIDO GOOGLE + HIGGSFIELD CLI — ✅ (2026-08-20)
- ✅ Vertex/AI Studio continuam como provedor padrão e como motor do Diretor, tradução e leitura das referências. A chave permanece somente no header; removido o fallback legado por query string.
- ✅ Higgsfield entrou somente como backend opcional da imagem final via `higgsfield-bridge.mjs`, preso ao localhost, com origin allowlist, subprocesso sem shell, allowlist de modelos, limite de payload e limpeza de temporários.
- ✅ Modelos expostos: Nano Banana Pro/2, Seedream 4.5/5 Lite/5 Pro e GPT Image 2. Custos por resolução aparecem antes do clique; 4K verificado: Nano Pro 4 cr, Nano 2 3 cr, GPT Image 2 12 cr.
- ✅ JPEG/WebP são normalizados para PNG no bridge para contornar o bug de assinatura do upload no CLI 0.1.40. Aspect ratios e resoluções são limitados pelo schema real de cada modelo.
- ✅ Job pago nunca recebe auto-retry nem fallback silencioso para outro provider. O saldo é atualizado depois da conclusão; sessão/credenciais Higgsfield nunca entram no browser.
- ✅ Testes: 5 suítes Fotograma antigas ALL PASS; `test-fotograma-higgsfield.mjs` cobre router/custos/constraints/no-retry/no-fallback/segredo; `test-higgsfield-bridge.mjs` cobre CORS/allowlist/validação/rotas. Health real confirmou plano Creator e sessão ativa sem gastar créditos.
#### 21.10 — IMAGE LAB VISÍVEL + FERRAMENTAS ESPECIALIZADAS — ✅ (2026-08-20)
- ✅ A interface deixou de esconder o trabalho no seletor de provider: sidebar permanente inspirada na anatomia observada no vídeo, com **Create, Multi Angle, Animation, Expand, Remove BG · beta** e atalho para Depth Map. O Create cinematográfico existente foi preservado dentro do novo shell.
- ✅ Só entraram fluxos auditados: Qwen Angles (giro/vertical/aproximação, 0,2 cr), Outpaint por aspect ratio (2 cr), Background Remover (1 cr, beta) e seis estilos autorais de retextura via modelo selecionável. Upscale e Expand por pixels continuam fora por falta de backend confiável.
- ✅ `shared/fotograma-tools.js` concentra custos, ratios e prompts próprios dos estilos; não copia system prompts privados da referência. Cada direção impõe fidelidade de identidade, pose, objetos, composição e iluminação.
- ✅ O bridge ganhou somente `/tool`, com allowlist fechada de operações e parâmetros; a origem oficial `tipo-steel.vercel.app` funciona sem configuração extra. Nenhum fluxo pago recebe retry automático.
- ✅ `test-fotograma-tools.mjs` prova navegação, upload/preview, quatro jobs mockados, prompts, payload sem credencial, reuso do bridge e mobile sem overflow. As 5 suítes antigas, provider Higgsfield e bridge continuam ALL PASS; health real confirmou Creator/1258.87 sem consumo.
#### 21.11 — GALERIA COMO WORKSPACE + LEGIBILIDADE — ✅ (2026-08-20)
- ✅ A imagem-demo gigante saiu da superfície visível. O centro do Fotograma agora é uma galeria vertical em grid, seguindo a hierarquia útil da referência: resultados primeiro, controles à esquerda.
- ✅ Grid ajustável de 1–4 colunas com persistência local e limite responsivo para nunca esmagar os cards; busca filtra prompt, cena, modelo e ferramenta; contador e vazio contextual completam a navegação.
...
#### 21.14 — GERAÇÃO DESACOPLADA + CREATE ENXUTO — ✅ (2026-08-24)
- ✅ Higgsfield no Create não depende mais da chave Google: bridge conectado libera o botão e a cena segue pelo motor determinístico local. Google permanece opcional para Diretor, tradução e análise automática de referências.
- ✅ Falha ou resposta inválida do Diretor não derruba mais a geração Google/Vertex; a cena sanitizada entra no compositor local e o modelo de imagem continua. Entradas de extração/injeção permanecem bloqueadas antes da fila.
- ✅ Vertex começa em `TEXT + IMAGE`, combinação aceita pela API, sem a tentativa inicial `IMAGE` incompatível. IDs estáveis Nano Banana Pro/2/2 Lite foram conferidos na documentação oficial atual.
- ✅ Presets visuais **Escala do plano** e **Ponto de vista** saíram da UI. Takes antigos continuam reproduzíveis, e a cena/Ficha/referência de composição ainda comandam o enquadramento.
- ✅ Create foi reorganizado em Cena → Estética → Referências → Motor da imagem; controles técnicos vivem em Direção avançada recolhida. Geração ganhou dock compacto, requisito visível e recuperação contextual do provedor.
- ✅ Oito suítes passaram; health real confirmou bridge Creator conectado, saldo 1255,87 e allowlist de seis modelos. Nenhuma geração real nem crédito foram consumidos nesta correção.
#### 21.15 — PROMPT LONGO + LOTE HIGGSFIELD + PROGRESSO NO GRID — ✅ (2026-08-25)
- ✅ O teto artificial de 1.800 caracteres saiu do caminho local. Cena agora aceita até **12.000 caracteres**, com contador e aviso visual; o prompt final continua protegido pelo limite de 24.000 do bridge e não é truncado silenciosamente.
- ✅ Create mostra **Imagens por geração 1–4** somente quando Higgsfield está selecionado. Quantidade persiste localmente, botão/custo mostram o total antes do clique e Google/Vertex continuam unitários.
- ✅ Cada saída é um job CLI independente e até quatro rodam em paralelo. Diretor, visão e Ficha são preparados uma única vez e compartilhados pelo lote; o bridge reserva quatro slots, isola temporários e rejeita um quinto job com 409 antes de chamar o CLI; ferramentas especializadas continuam exclusivas enquanto há geração ativa.
- ✅ Todos os cartões entram imediatamente no topo da galeria e mostram modelo, índice do lote, formato, spinner, barra, percentual e tempo restante **estimados**. A estimativa usa histórico local por modelo, aprende com jobs concluídos e fica limitada a 92% até o arquivo chegar, sem fingir telemetria que o CLI não fornece.
- ✅ Test-first sem créditos: `test-fotograma-batch.mjs` prova cena de 6.043 caracteres, três payloads completos em paralelo, custo, persistência, estados no grid e mobile; `test-higgsfield-bridge-concurrency.mjs` prova quatro processos simultâneos + bloqueio do quinto com CLI simulado. Dez suítes Fotograma/bridge passaram, captura de progresso revisada e nenhuma geração real foi disparada.
#### 21.16 — HOTFIX DO TIMEOUT HIGGSFIELD — ✅ (2026-08-25)
- ✅ Causa reproduzida: `connectHiggsfieldAttempt()` criava o adapter inteiro com `timeoutMs: 30_000`; o mesmo relógio correto para o health abortava qualquer geração após 30 segundos, enquanto o CLI seguia processando/cobrando no bridge.
- ✅ Adapter agora separa `healthTimeoutMs` (30s) de `timeoutMs` da geração (23min). O browser espera um minuto além do limite interno do bridge/CLI (22min), então recebe tanto o arquivo quanto um eventual erro real do servidor.
- ✅ Cache-bust do provider atualizado e regressão permanente confirma `health=30.000` e `generation=1.380.000`. O job observado após o falso timeout concluiu no Higgsfield; nenhum retry foi disparado durante o diagnóstico.
#### 21.17 — FILA CONTÍNUA + HISTÓRICO LOCAL COMPLETO — ✅ (2026-08-25)
- ✅ `Revelar` não bloqueia mais durante um job. Cada clique captura a cena/configuração atuais e acrescenta outro lote de 1–4 imagens sem apagar nem esperar o lote anterior; o botão comunica `Revelar +N` enquanto há trabalho.
- ✅ O scheduler mantém no máximo quatro gerações simultâneas no bridge e deixa o excedente visível no grid como `na fila · posição N`. Limite seguro: 48 imagens entre jobs ativos e aguardando; conforme cada slot termina, a próxima começa automaticamente.
- ✅ Auditoria da persistência encontrou dois cortes artificiais: o reload mostrava só 30 registros e a sessão removia takes antigos a partir de 120. Ambos saíram. A galeria agora restaura todos os blobs existentes no IndexedDB, usa imagens lazy/async para históricos grandes e só exclui pela ação explícita do usuário.
- ✅ A página solicita `navigator.storage.persist()` e mostra `salva neste navegador`/`armazenamento local protegido`; falha por quota vira aviso claro. Continua sendo persistência local do mesmo domínio/browser/perfil, não nuvem: limpar dados do site, usar janela privada ou trocar de aparelho/perfil perde o histórico.
- ✅ `test-fotograma-batch.mjs` prova cliques consecutivos (4 ativos + excedente na fila), 12 arquivos independentes, Diretor exatamente uma vez por clique mesmo quando o lote entra parcialmente na fila, recarga com 38 imagens, lazy decoding, badge de armazenamento e mobile. Regressões Higgsfield, ferramentas e prompts passaram; captura da fila revisada. Nenhum crédito real consumido.
#### 21.18 — HIGGSFIELD SEM CÓDIGO + CONEXÃO SEMPRE ATIVA — ✅ (2026-09-05)
- ✅ Causa raiz fechada: o bridge estava desligado, não existia serviço de login, a sessão havia expirado e `/usr/local/bin/higgsfield` ainda era o CLI 0.1.40. O CLI atual 1.1.24 foi instalado em `~/bin/higgsfield` e autenticado pelo OAuth oficial com callback local, sem copiar código ou expor credencial à Tipó.
- ✅ `support/macos/install-higgsfield-bridge.zsh` instala um LaunchAgent privado com `RunAtLoad`, `KeepAlive`, HOME/PATH explícitos, CLI correto, loopback e logs locais. A atualização só anuncia sucesso depois de `/live`; se o novo serviço não subir, restaura o plist anterior. Reinício forçado de prova mudou o PID e o serviço voltou saudável sozinho.
- ✅ A página agora mantém um supervisor de conexão: tentativa no boot, backoff limitado, heartbeat, retomada em foco/pageshow/rede e invalidação correta após falha de transporte. URL e teste manual ficam apenas no diagnóstico avançado; sessão expirada mostra **Entrar** e aciona `/auth/login` uma única vez.
- ✅ Segurança financeira preservada: probes são deduplicados, erros 400/409/429 não derrubam a sessão e `/generate`/`/tool` jamais recebem retry automático. Se a conexão cai antes do POST, a fila pausa visível e retoma somente os jobs comprovadamente não enviados; uma resposta perdida depois do POST nunca é repetida.
- ✅ Scheduler isolado por provedor: até 4 Higgsfield e 1 Google podem avançar sem um bloquear o outro. IDs são monotônicos mesmo com relógio congelado, reentrada conserva FIFO e erro atrasado de um adapter antigo não invalida uma conexão nova.
- ✅ OAuth bloqueia geração/ferramenta concorrente, sessão expirada não faz polling nem interrompe o provedor Google e troca de URL descarta resultado antigo. POST sem Origin ou fora da allowlist é rejeitado; apenas a origem oficial é padrão e desenvolvimento exige opt-in; `/live` separa processo ativo de conta pronta.
- ✅ Regressões mockadas, concorrência, bridge e smoke real no Chromium passaram. O teste real concedeu somente a permissão de rede local ao origin, conectou no plano Creator e habilitou a geração sem chave Google, sem pageerror e sem consumir créditos.
