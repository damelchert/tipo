...
- **22.2 Tipó Studio (o projeto grande — NÃO começado)**: página nova studio.html com o modelo de stack do Sketch — chain WebGL2 ping-pong (2 framebuffers, cada efeito = fragment shader com uInputTexture, composição chroma-key dist euclidiana+smoothstep, 1 rAF síncrono, quality tiers) encadeando versões shader dos efeitos TIPÓ: halftone/reticula, Bayer dither, gradient map, pixelate, glitch (rgb shift/slices), grain/leak, threshold/posterize. Inputs: imagem/vídeo/webcam/texto (demos brand). Efeitos sequenciais (error diffusion, pixel sort) ficam FORA do chain v1 (ou pass CPU depois). Rack de pós-efeitos global (Adjustments/Aberration/Glow/Grain) nasce de graça como passes. UI: canvas com preview central + stack de cards reordenáveis; canvas infinito/pan-zoom só se sobrar tempo (cosmético).
- 22.3 (ideias, sem compromisso): TipoAudio nas gravações (áudio no MP4 via AudioEncoder — arquitetura do 16.4 já existe pra vídeo-fonte); BPM phase-lock; export com re-play sincronizado.
